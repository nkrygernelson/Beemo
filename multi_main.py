...
GOOGLE_DRIVE = False # SET THIS TO True TO ENABLE GOOGLE DRIVE SAVING
MAC = True

DRIVE_MOUNT_POINT = '/content/drive'
# *** CUSTOMIZE THIS TO YOUR PREFERRED GDRIVE FOLDER ***
YOUR_PROJECT_GDRIVE_FOLDER = 'AI_Bandgap_Project/MultiFidelityOutput'
DRIVE_BASE_SAVE_PATH = os.path.join(DRIVE_MOUNT_POINT, 'MyDrive', YOUR_PROJECT_GDRIVE_FOLDER)

# This will be "" if not using Google Drive, or the DRIVE_BASE_SAVE_PATH if using it.
save_prefix = ""


def prepare_datasets_for_multifidelity(subsmaple_dict = None):
    """
    ...
    for fidelity_name, fidelity_id in FIDELITY_MAP.items():
        ...
        data_file_path = os.path.join(save_prefix, 'data', 'train', f'{fidelity_name}.csv')
        
        # Fallback to local path if file not found at prefixed path (e.g., if inputs are always local)
        if not os.path.exists(data_file_path) and save_prefix: # only try local if prefixed path failed AND prefix exists
            print(f"File not found at {data_file_path}, trying local path 'data/train/{fidelity_name}.csv'")
            local_path_check = f'data/train/{fidelity_name}.csv'
            if os.path.exists(local_path_check):
                 data_file_path = local_path_check
            else:
                print(f"ERROR: Data file for {fidelity_name} not found at {data_file_path} or {local_path_check}")
                print("Please ensure your data .csv files (e.g., GGA.csv) are in the correct location:")
                print(f"  - If GOOGLE_DRIVE=True: In '{os.path.join(DRIVE_BASE_SAVE_PATH, 'data', 'train')}' on your Google Drive.")
                print(f"  - If GOOGLE_DRIVE=False: In the local './data/train/' directory.")
                print(f"Skipping {fidelity_name} dataset.")
                continue
        elif not os.path.exists(data_file_path) and not save_prefix: # Not using drive and file not found locally
            print(f"ERROR: Data file for {fidelity_name} not found at {data_file_path}")
            print("Please ensure your data .csv files are in the local './data/train/' directory.")
            print(f"Skipping {fidelity_name} dataset.")
            continue
        ...
        if subsmaple_dict:
            sample_frac = subsmaple_dict.get(fidelity_name, 1)
    ...
    if combined_train_df.empty and FIDELITY_MAP:
...
def train_multifidelity_model(combined_train_df, model_params = None, pooling_type = "gated"):
    """
    ...
    data_dir_path = os.path.join(save_prefix, "data")
    predictions_dir_path = os.path.join(save_prefix, "predictions", "multifidelity")

    # Save the combined training dataset
    os.makedirs(data_dir_path, exist_ok=True)
    combined_train_csv_path = os.path.join(data_dir_path, "multifidelity_train.csv")
    ...
    best_model_path = os.path.join(predictions_dir_path, f"best_model_{pooling_type}.pt")
    ...
    plt.legend()
    plt.grid(True)
    plot_loss_path = os.path.join(predictions_dir_path, f"train_val_loss_{pooling_type}.png")
...
def plot_fidelity_results(predictions, targets, fidelity_name, metrics):
    ...
    fidelity_plots_dir = os.path.join(save_prefix, "predictions", "multifidelity", "fidelity_plots")
    os.makedirs(fidelity_plots_dir, exist_ok=True)

    plt.figure(figsize=(8, 8))
    plt.scatter(targets, predictions, alpha=0.5)

    min_val = min(min(targets, default=0), min(predictions, default=0)) # Added default for empty lists
    max_val = max(max(targets, default=1), max(predictions, default=1)) # Added default for empty lists
    ...
    plot_file_path = os.path.join(fidelity_plots_dir, f"{fidelity_name}_actual_vs_predicted.png")
    ...
    preds_csv_path = os.path.join(fidelity_plots_dir, f"{fidelity_name}_predictions.csv")
...
def create_summary_plot(results):
    ...
    os.makedirs(summary_dir, exist_ok=True) # Already created in train_multifidelity_model, but good to be sure
...
def run_multifidelity_experiments(subsample_dict = None):
    """
    Run the complete multi-fidelity training and evaluation workflow.
    """
    global save_prefix # Crucial for functions below to know the correct base path
    ...
    if GOOGLE_DRIVE:
        try:
            ...
            save_prefix = DRIVE_BASE_SAVE_PATH # Set the global prefix
            print(f"Successfully mounted Google Drive. Saving outputs to: {save_prefix}")
            # Create base output directories on Drive
            os.makedirs(os.path.join(save_prefix, "data", "train"), exist_ok=True) # For input data structure
            os.makedirs(os.path.join(save_prefix, "predictions", "multifidelity", "fidelity_plots"), exist_ok=True)
        except ImportError:
    ...
    print("="*50 + "\n")
    combined_train_df, test_datasets = prepare_datasets_for_multifidelity(subsmaple_dict=subsample_dict)
    ...
    model, mean, std, preprocess = train_multifidelity_model(combined_train_df, model_params=model_params_global, pooling_type=model_params_global.get('pooling_type','gated'))
...
if __name__ == "__main__":
    ...
    model_params_global = { 
        ...
        "dropout": 0.1,
        "pooling_type": "gated"}
    subsample_dict = {"GGA": 0.001, "GGAU": 0.001,"SCAN": 0.001,"GLLBSC": 0.001 }

    results, plot_paths = run_multifidelity_experiments(subsample_dict=subsample_dict)

    print("\n--- Experiment Finished ---")
    if GOOGLE_DRIVE and save_prefix: # Check if save_prefix was successfully set
        print(f"All outputs should be saved in your Google Drive under: {save_prefix}")
    elif GOOGLE_DRIVE and not save_prefix:
        print("GOOGLE_DRIVE was True, but an issue occurred. Outputs saved locally (if any).")
    else:
        print(f"All outputs saved locally (current directory structure). Base prefix was: '{save_prefix}'")